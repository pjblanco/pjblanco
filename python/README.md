...
This is a small, security-first remote-support prototype. It provides **read-only screen sharing** over TLS after the person at the host explicitly approves the session.

It intentionally does **not** provide:

- stealth installation or persistence;
- security-control bypasses or privilege escalation;
...
- The viewer pins the host certificate fingerprint, protecting against a relay or man-in-the-middle when the fingerprint is verified out of band.
- The host displays a consent prompt before any frame is captured or sent.
- Only one session is allowed at a time.
- The host shows a visible session banner in the terminal and records JSON audit events.
- Stopping either side ends the session. No background service or installer is included.

## Requirements

Python 3.11+ and OpenSSL are required. Pillow is installed with the package and is used for screen capture/display.
...
Run this once on the host computer. The certificate and private key should be readable only by the host user.

```bash
remote-support cert --cert host-cert.pem --key host-key.pem
chmod 600 host-key.pem
```

On Windows, protect the key using the filesystem ACLs instead of `chmod`.
...
The host prints an address, a certificate fingerprint, and a one-time pairing code. Share those values with the trusted viewer through a separate channel. Do not post them publicly. Approve the viewer at the host prompt when the identity is expected.
...
```bash
remote-support viewer ... --headless --output ./session-output
```
